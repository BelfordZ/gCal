// Google Calendar Content Aggregation
// Author: Zach Belford
// Calvis Loader by google
// Version 1.0
...
function getCal(eids) {
    elementIDs = eids;
    calvis.ready(main);
}

function main() {
    var calId = elementIDs.calID;
    var calendar = new calvis.Calendar();

    // set the CSS IDs for various visual components for the calendar container
    calendar.setCalendarBody(elementIDs.bodyID);
    calendar.setStatusControl(elementIDs.statusID);
    calendar.setNavControl(elementIDs.navID);
    calendar.setViewControl(elementIDs.viewControlId);
    calendar.setEventCallback('mouseover', displayEvent);

    calendar.setPublicCalendar(calId);
    calendar.setLoginControl(elementIDs.loginID);  

    calendar.setDefaultView(elementIDs.view);
...
function displayEvent(event) {    
    ...
    var eventHtml = [];
    eventHtml.push('<p><b>Title:</b> ');
    eventHtml.push(title);
    eventHtml.push('</p>');
    
    eventHtml.push('<p><b>Date:</b> ');
    eventHtml.push(date);
    eventHtml.push('</p>');

    for (var keyz in content){
        if (content.hasOwnProperty(keyz)) {
            eventHtml.push('<p><b>' + keyz + ': </b>');
            eventHtml.push(content[keyz]);
            eventHtml.push('</p>');
        }
    }

  document.getElementById(elementIDs.eventID).innerHTML = eventHtml.join('');
} 